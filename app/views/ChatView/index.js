...
class ChatView extends React.Component {
  ...
  constructor(props) {
    ...
    this._finishedLoadingFileLister = SoundPlayer.addEventListener(
      'FinishedLoadingFile',
      () => {
        this.canSoundPlay = true;
      },
    );
    this._finishedLoadingLister = SoundPlayer.addEventListener(
      'FinishedLoading',
      () => {
        this.canSoundPlay = true;
      },
    );
    this._finishedPlayingLister = SoundPlayer.addEventListener(
      'FinishedPlaying',
      () => {
        this.canSoundPlay = true;
      },
  ...
  init = async () => {
    ...
    let messagesSubscribe = await firestore().collection(
      firebaseSdk.TBL_MESSAGE,
    );
    this.unSubscribeMessage = messagesSubscribe.onSnapshot(
      async querySnapShot => {
        ...
        if (
          ...
          this.state.messages.length !== list.length
        ) {
          this.playSound();
        }
        if (this.mounted) {
        ...
        <>
          ...
          <View style={styles.dateSeparator}>
            <Text style={styles.dateSepText}>
              {moment(dateSeparator).format('LL')}
            </Text>
          </View>
        </>
      );
      ...
      <SafeAreaView>
        ...
        <FlatList
          ...
          renderItem={({item, index}) =>
            this.renderItem(item, messages[index + 1], index)
          }
        ...
        {showActiveImage && (
          <TouchableOpacity
            ...
            onPress={() => this.setState({showActiveImage: null})}>
