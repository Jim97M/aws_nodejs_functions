...
class EditPostView extends React.Component {
  ...
  takeThumbnail = () => {
    ...
    createThumbnail({url: this.state.file_path, timestamp: 100}).then(
      thumbnail => {
  ...
  renderForm = () => {
    const {type, text, file_path, photo, video, thumbnail, playing} =
      this.state;
